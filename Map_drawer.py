# -*- coding: utf-8 -*-
"""
Created on Sat May  9 14:59:38 2020
...
# Importing the Kivy packages to make map
...
from ai_car import DQN #DEEP Q NETWORK
...
# action = 0 => no rotation, action = 1 => rotate 20 degres(right), action = 2 => rotate -20 degres(left)
...
def init():
    global sand # sand is an array that has as many cells as our graphic interface has pixels. Each cell has a 1 if there is sand, 0 otherwise.
    ...
    global first_update 
    first_update = False # trick to initialize the map only once= False # trick to initialize the map only once
...
class Game(Widget):
    ...
    def stop_car(self): # stoping the car 
        self.car.velocity = Vector(0, 0) #velocity decreased to zero
    
    def start_car(self): #restarting the car
